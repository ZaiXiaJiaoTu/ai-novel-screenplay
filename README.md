...
- 剧情事件拆分：每 3 章为一个批次，支持单次拆分、全部拆分和安全停止；提示词提取"可独立用于剧本场景编排的剧情节点"，支持人物别名（`aliases`）提取，事件可编辑、删除，已用于剧集后锁定。
- 人物档案：随剧情拆分增量提取人物结构化事实（`facts`/`fact_type`），支持手动编辑和 AI 一键整合档案；人物事实按时序排列，整合时区分历史状态与当前状态；别名匹配避免重复创建人物。
- 分集剧本生成：以剧情事件为范围、原文章节为细节、人物档案为约束，按未锁定剧情事件生成单集或全部剧集；支持安全停止；后端固定规范化 YAML metadata 和字段类型（`action`=字符串数组、`dialogue`=对象数组、`transition`=字符串）。
- 生成修复链路：生成 → YAML 解析 → 业务校验 → 失败则调用修复任务（最多一次）→ 再校验 → 仍失败使用兜底；修复支持 YAML 解析错误和业务校验错误。
- 剧本编辑：支持中文表单编辑和 YAML 源码编辑。
- 剧本导出：支持单集/全集导出，格式为 YAML 或渲染后的 TXT。
- 大模型配置：支持 OpenAI 兼容 API 配置、加密保存 API Key、测试连接和默认配置。
- 提示词模板：4 个默认模板（剧情事件拆分、单集剧本生成、人物档案整合、单集剧本修复），支持筛选、编辑、启停、版本查看和回滚。
